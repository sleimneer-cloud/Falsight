...
int main(int argc, char* argv[]) {
    ...
    log_main("StreamSender 시작 (모든 카메라 스트리밍 ON)");
    ...
    auto distributor = [&](int cam_index) {
        ...
        while (main_queues[cam_index]->pop(frame)) {
            ...
            display_queues[cam_index]->push(frame);   // 디스플레이
            storage_queues[cam_index]->push(frame);   // 저장

            if (frame.has_motion)                     // AI (모션 시만)
                shared_zmq_queue.push(frame);

            if (stream_sender->is_camera_enabled(frame.camera_id))  // 스트리밍
    ...
    // Step 2. 메인 큐 shutdown → 카메라 push 블로킹 해제
    // Step 3. 카메라 중지     → push 블로킹 없이 즉시 종료
    // Step 4. 나머지 큐 shutdown → 분배/저장/스트림 스레드 종료
    // Step 5. 워커 스레드 join → 모든 스레드 완전 종료 확인
    // Step 6. 송신 클래스 정리 → 소켓 닫기 (반드시 마지막)
    //--------------------------------------------------------------------------
    log_main("========== 종료 처리 시작 ==========");
